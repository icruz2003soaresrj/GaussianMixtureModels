...
class GaussianMixtureModel:
    ...
    def __init__(self, X : np.ndarray, M : int):
        ...
        self.E_Q_X_mu_Lambda = np.zeros(shape = (self.N, self.M))
    ...
    def initialize_parameters(self) -> None:
        ...
        self.alpha = np.repeat(a = self.alpha_0, repeats = self.M)

        self.tau = np.repeat(a = self.tau_0, repeats = self.M)

        self.mu = kmeans_plusplus(X = self.X, n_clusters = self.M)[0]

        self.nu = np.repeat(a = self.nu_0, repeats = self.M)

        self.Phi = np.repeat(a = np.expand_dims(self.Sigma_0, axis = 0), repeats = self.M, axis = 0)
    ...
    def update_E_log_det_Lambda(self) -> None:
        ...
        for d in range(1, self.D):
            ...
            self.E_log_det_Lambda += psi((self.nu + 1 - d)/2)

    def update_E_Q_X_mu_Lambda(self) -> None:

        for m in range(self.M):

            for n in range(self.N):

                self.E_Q_X_mu_Lambda[n, m] = self.nu[m]*(self.X[n] - self.mu[m]).T @ self.Psi[m] @ (self.X[n] - self.mu[m])

                self.E_Q_X_mu_Lambda[n, m] += self.D/self.tau[m]
    ...
    def update_gamma(self) -> None:
        ...
        for m in range(self.M):
            ...
            for n in range(self.N):
                ...
                self.gamma[n, m] -= self.E_Q_X_mu_Lambda[n, m]/2
    ...
    def estimates_Z(self) -> None:
        ...
        self.Z = np.argmax(a = self.gamma, axis = 1) + 1
    ...
    def estimates_Lambda(self) -> None:
        ...
        self.Lambda = np.expand_dims(self.nu, axis = (1, 2))*self.Phi
    ...
    def optimize_parameters(self, MAX : int = 10000, TOL = 1e-6) -> None:
